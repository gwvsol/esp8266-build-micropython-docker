...
RUN set -eux \
    && apt-get update \
    && apt-get upgrade -y \
    && apt-get install -y python python-dev python-serial python-pip \
...
USER root
RUN ln -s ${HOME_DIR}/micropython/mpy-cross/mpy-cross /usr/local/bin/ \
    && cp -a ${HOME_DIR}/${TOOLS}/xtensa-lx106-elf /usr/local/ \
    && rm -fr ${HOME_DIR}/${TOOLS}
